...
module.exports = function transformer(fileInfo, api) {
  ...
  function getHookType(hookName) {
    ...
    if (hookName.match(/use(Create|Update|Delete|Add|Remove|Set|Save|Post|Put|Patch)/)) {
      return 'mutation';
    }
    return 'query';
  }
  ...
  root.find(j.ImportDeclaration)
    ...
    .forEach(path => {
      ...
      path.node.specifiers.forEach(spec => {
        if (j.ImportSpecifier.check(spec) && spec.imported.name.startsWith('use')) {
  ...
  if (hasModifications) {
      ...
      .forEach(path => {
        ...
        if (optionsToAdd) {
          ...
          optionsToAdd.forEach(optionName => {
            newSpecifiers.push(
              j.importSpecifier(j.identifier(optionName))
            );
          });
    ...
    if (existingReactQueryImport.length > 0) {
      ...
      const newImport = j.importDeclaration(imports, j.literal('@tanstack/react-query'));
  ...
  return hasModifications ? root.toSource({
    quote: 'single',
    trailingComma: true,
  }) : fileInfo.source;
};
